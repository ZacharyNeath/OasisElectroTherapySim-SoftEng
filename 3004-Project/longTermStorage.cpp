...
LongTermStorage::~LongTermStorage(){
    cout<< "    --LongTermStorage dtor"<<endl;
}
...
bool LongTermStorage::loadRecords(){
    ...
    for( auto v : content){
        ...
        Session* s = createSession(o["Group"].toString(),o["Type"].toString(),o["Duration"].toInt(),o["Frequency"].toInt(),o["CES Mode"].toBool(),o["Date"].toString());
        records.push_back(s);
...
bool LongTermStorage::saveRecord(Session* s){
    ...
    //Create QJsonArrat out of collection
    QJsonArray content;
    for(auto v : records){
        ...
        o.insert("Duration",v->getDuration());
...
Session* LongTermStorage::createSession(const QString& groupString, const QString& typeString, const int duration, const int frequency, const bool ces, const QString date){
    QDateTime dt = QDateTime::fromString(date,"ddd MMM dd HH:mm:ss yyyy");
    return new Session(groupString, typeString, duration, frequency, ces, dt);
}
